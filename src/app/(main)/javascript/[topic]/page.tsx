...
export default function JavaScriptTopicPage() {
  ...
  const prevTopic = topicIndex > 0 ? javascriptTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex < javascriptTopics.length - 1 ? javascriptTopics[topicIndex + 1] : null;
  const topicProgressKey = topic
    ? getTopicProgressKey("javascript", topic.slug)
    : null;
  ...
  return (
    <div className="flex">
      <TopicSidebar
        ...
        sidebarTitle="JavaScript Topics"
        sidebarDescription="Master JavaScript fundamentals"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionCompletedCount}/{javascriptTopics.length} topics completed
                  </span>
                </div>
          ...
          <Tabs defaultValue="concept" className="space-y-6">
            ...
            <TabsContent value="visualization">
              <JavaScriptVisualizationSection topicSlug={topic.slug} />
            </TabsContent>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-8">
            {prevTopic ? (
              <Link href={`/javascript/${prevTopic.slug}`} className="flex-1">
            ...
            {nextTopic ? (
              <Link href={`/javascript/${nextTopic.slug}`} className="flex-1">
                <Card className="h-full hover:bg-accent/50 transition-colors group cursor-pointer">
