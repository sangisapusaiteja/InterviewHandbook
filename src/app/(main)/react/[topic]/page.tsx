...
export default function ReactTopicPage() {
  ...
  const prevTopic = topicIndex > 0 ? reactTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex < reactTopics.length - 1 ? reactTopics[topicIndex + 1] : null;
  const topicProgressKey = topic
    ? getTopicProgressKey("react", topic.slug)
    : null;
  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)]">
      <TopicSidebar
        ...
        sidebarTitle="React Topics"
        sidebarDescription="Master React fundamentals"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionCompletedCount}/{reactTopics.length} topics completed
                  </span>
                </div>
          ...
          <Tabs defaultValue="concept" className="space-y-4 sm:space-y-6">
            ...
            <TabsContent value="visualization">
              <ReactVisualizationSection topicSlug={topic.slug} />
            </TabsContent>
            ...
            <TabsContent value="code">
              <div className="space-y-4">
                ...
                />
                <ReactPreview
                  code={topic.code.defaultCode}
                  files={topic.code.files}
                />
              </div>
            </TabsContent>
          </Tabs>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-6 sm:pb-8">
            {prevTopic ? (
              <Link href={`/react/${prevTopic.slug}`} className="flex-1">
            ...
            {nextTopic ? (
              <Link href={`/react/${nextTopic.slug}`} className="flex-1">
                <Card className="h-full hover:bg-accent/50 transition-colors group cursor-pointer">
      ...
      <TopicAssistant topic={topic} sectionTitle="React" />
    </div>
  );
}
