...
export default function PostgreSQLTopicPage() {
  ...
  const prevTopic = topicIndex > 0 ? postgresqlTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex < postgresqlTopics.length - 1 ? postgresqlTopics[topicIndex + 1] : null;
  const topicProgressKey = topic
    ? getTopicProgressKey("postgresql", topic.slug)
    : null;
  ...
  return (
    <div className="flex">
      <TopicSidebar
        ...
        sidebarTitle="PostgreSQL Topics"
        sidebarDescription="Master PostgreSQL fundamentals"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionCompletedCount}/{postgresqlTopics.length} topics completed
                  </span>
                </div>
          ...
          <Tabs defaultValue="concept" className="space-y-6">
            ...
            <TabsContent value="visualization">
              <PostgreSQLVisualizationSection topicSlug={topic.slug} />
            </TabsContent>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-8">
            {prevTopic ? (
              <Link href={`/postgresql/${prevTopic.slug}`} className="flex-1">
            ...
            {nextTopic ? (
              <Link href={`/postgresql/${nextTopic.slug}`} className="flex-1">
                <Card className="h-full hover:bg-accent/50 transition-colors group cursor-pointer">
