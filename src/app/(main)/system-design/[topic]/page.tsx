...
export default function SystemDesignTopicPage() {
  ...
  const prevTopic = topicIndex > 0 ? systemDesignTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex < systemDesignTopics.length - 1
      ? systemDesignTopics[topicIndex + 1]
      : null;
  const topicProgressKey = topic
    ? getTopicProgressKey("system-design", topic.slug)
    : null;
  ...
  return (
    <div className="flex">
      <TopicSidebar
        ...
        sidebarDescription="Learn architecture tradeoffs and large-scale system patterns"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionCompletedCount}/{systemDesignTopics.length} topics completed
                  </span>
                </div>
          ...
          <Tabs defaultValue="concept" className="space-y-6">
            ...
            <TabsContent value="visualization">
              <SystemDesignVisualizationSection
                topic={topic}
                modules={systemDesignModules}
              />
            </TabsContent>
            ...
            <TabsContent value="checklist">
              <SystemDesignChecklistSection topic={topic} />
            </TabsContent>
          </Tabs>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-8">
            {prevTopic ? (
              <Link href={`/system-design/${prevTopic.slug}`} className="flex-1">
            ...
            {nextTopic ? (
              <Link href={`/system-design/${nextTopic.slug}`} className="flex-1">
                <Card className="h-full hover:bg-accent/50 transition-colors group cursor-pointer">
