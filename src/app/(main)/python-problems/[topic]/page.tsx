...
export default function PythonProblemTopicPage() {
  ...
  const topicIndex = pythonProblemTopics.findIndex((t) => t.slug === topicSlug);
  const topic = topicIndex >= 0 ? pythonProblemTopics[topicIndex] : undefined;
  const prevTopic = topicIndex > 0 ? pythonProblemTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex < pythonProblemTopics.length - 1
      ? pythonProblemTopics[topicIndex + 1]
      : null;
  const topicProgressKey = topic
    ? getTopicProgressKey("python-problems", topic.slug)
    : null;
  ...
  return (
    <div className="flex">
      <TopicSidebar
        ...
        sidebarTitle="Practice Problems"
        sidebarDescription="Python beginner practice"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionCompletedCount}/{pythonProblemTopics.length} problems completed
                  </span>
                </div>
          ...
          <Tabs defaultValue="concept" className="space-y-6">
            ...
            <TabsContent value="visualization">
              <PythonProblemsVisualizationSection topicSlug={topic.slug} />
            </TabsContent>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-8">
            {prevTopic ? (
              <Link href={`/python-problems/${prevTopic.slug}`} className="flex-1">
            ...
            {nextTopic ? (
              <Link href={`/python-problems/${nextTopic.slug}`} className="flex-1">
                <Card className="h-full hover:bg-accent/50 transition-colors group cursor-pointer">
