...
export default function TechnicalQuestionTopicPage() {
  ...
  const prevTopic = topicIndex > 0 ? technicalTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex < technicalTopics.length - 1 ? technicalTopics[topicIndex + 1] : null;
  const topicProgressKey = topic
    ? getTopicProgressKey("technical-questions", topic.slug)
    : null;
  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)]">
      <TopicSidebar
        ...
        basePath="/technical-questions"
        modules={technicalModules}
        sidebarTitle="Technical Questions"
        sidebarDescription="JavaScript, React, TypeScript &amp; NextJS"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {sectionCompletedCount}/{technicalTopics.length} topics completed
                  </span>
                </div>
          ...
          <Tabs defaultValue="concept" className="space-y-4 sm:space-y-6">
            ...
            <TabsContent value="code">
              {topic.code.files && topic.code.files.length > 0 ? (
                <div className="space-y-6">
                  {topic.code.files.map((file) => (
                    <div key={file.name}>
                      <h3 className="text-sm font-semibold text-muted-foreground mb-2">
                        {file.name}
                      </h3>
                      <CodeEditorSection
                        defaultCode={file.content}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-6 sm:pb-8">
            {prevTopic ? (
              <Link href={`/technical-questions/${prevTopic.slug}`} className="flex-1">
            ...
            {nextTopic ? (
              <Link href={`/technical-questions/${nextTopic.slug}`} className="flex-1">
                <Card className="h-full hover:bg-accent/50 transition-colors group cursor-pointer">
      ...
      <TopicAssistant topic={topic} sectionTitle="Technical Questions" />
    </div>
  );
}
