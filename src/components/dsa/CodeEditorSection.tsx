...
export function CodeEditorSection({
  ...
  const runCode = useCallback(async () => {
    ...
    const logs: string[] = [];
    try {
      // JSX can't run in new Function — show a helpful message
      if (/<[A-Z]/.test(code) || /<\/(div|button|h1|h2|h3|input|p|span|strong|nav|header|section)>/.test(code)) {
        setOutput([
          "// This code contains JSX and cannot run in the browser console.",
          "// Click the 'Render Preview' button below to see it in action.",
        ]);
        setIsRunning(false);
        return;
      }
      ...
      // Strip import/export statements — they aren't supported in new Function
        ...
        <CardHeader className="pb-3 shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <Button variant="ghost" size="sm" onClick={copyCode} className="h-8 px-2 sm:px-3">
                {copied ? (
              ...
              <Button variant="ghost" size="sm" onClick={resetCode} className="h-8 px-2 sm:px-3">
                <RotateCcw className="h-3.5 w-3.5 sm:mr-1" />
                <span className="hidden sm:inline">Reset</span>
              </Button>
              <Button size="sm" onClick={runCode} disabled={isRunning} className="h-8 px-2 sm:px-3">
