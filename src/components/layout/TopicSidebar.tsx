...
const moduleIconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  "js-variables-scope": Braces,
  "js-functions": FunctionSquare,
  "js-async": Timer,
  "js-array-methods": List,
  "js-objects": Boxes,
  "js-modern": Zap,
  "js-performance": Timer,
  "react-fundamentals": LayoutGrid,
  "react-hooks": Code,
          ...
          <ScrollArea ref={scrollRef} type="always" className="h-full">
            <nav className={cn("py-2", iconOnly ? "px-2" : "px-3 pb-6")}>
              ...
              {iconOnly ? (
                ...
                /* ── EXPANDED: grouped by category then module ── */
                <div className="space-y-4">
                  {Array.from(groupedModules.entries()).map(([category, catModules]) => (
                    <div key={category}>
                      ...
                      <div className="space-y-0.5">
                        {catModules.map((module) => {
                          ...
                          const isEmpty = module.topicIds.length === 0;
                          const moduleCompleted = moduleTopics.filter((t) =>
                            completedTopics.includes(getTopicProgressKey(sectionSlug, t.slug))
                          ).length;
                          const isActiveModule = module.id === activeModuleId;
                          const dotColor = DIFFICULTY_DOT[module.difficulty] ?? "bg-muted-foreground";
                          return (
                            <div key={module.id}>
                              <button
                                ...
                                className={cn(
                                  ...
                                  isEmpty ? "opacity-40 cursor-not-allowed" : isActiveModule ? "bg-primary/5 hover:bg-primary/10" : "hover:bg-accent/60 active:bg-accent"
                                )}
                              >
                                {(() => {
                                  const ModuleIcon = moduleIconMap[module.id] || Circle;
                                  return (
                                    <span className={cn("shrink-0 w-6 h-6 rounded-md border flex items-center justify-center", isActiveModule ? "bg-primary/15 border-primary/30" : "bg-muted")}>
                                      <ModuleIcon className={cn("h-3.5 w-3.5", isActiveModule ? "text-primary" : "text-muted-foreground")} />
                                    </span>
                                  );
                                })()}
                                <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                  <span className={cn("shrink-0 h-1.5 w-1.5 rounded-full", dotColor)} />
                                  <span className={cn("text-[12px] font-semibold leading-tight truncate", isActiveModule ? "text-primary" : "text-foreground")}>{module.title}</span>
                                </div>
                                <div className="shrink-0 flex items-center gap-1">
                                  {isEmpty ? (
                                    ...
                                    <>
                                      <span className="text-[10px] tabular-nums text-muted-foreground">{moduleCompleted}/{moduleTopics.length}</span>
                                  ...
                                  >
                                    <div className="ml-4 pl-3 border-l border-border/50 mt-0.5 mb-1 space-y-0.5">
                                      {moduleTopics.map((topic) => {
                                        ...
                                        const isActive = pathname === `${basePath}/${topic.slug}`;
                                        const isCompleted = completedTopics.includes(getTopicProgressKey(sectionSlug, topic.slug));
                                        return (
                                          <Link
                                            key={topic.id}
                                            ...
                                            className={cn("flex items-center gap-2.5 rounded-lg px-2.5 py-2 relative", "transition-colors duration-150", isActive ? "bg-primary/10" : "hover:bg-accent/70 active:bg-accent")}
                                          >
                                            {isActive && (
                                              <motion.div
                                                layoutId="activeTab"
                                            ...
                                            <div className={cn("shrink-0 w-7 h-7 rounded-lg flex items-center justify-center", "transition-colors duration-150", isActive ? "bg-primary/15" : isCompleted ? "bg-emerald-500/10" : "bg-muted")}>
                                              <Icon className={cn("h-[14px] w-[14px]", isActive ? "text-primary" : isCompleted ? "text-emerald-500" : "text-muted-foreground")} />
                                            </div>
                                            <p className={cn("flex-1 min-w-0 text-[12px] font-medium leading-tight truncate", isActive ? "text-primary" : "text-foreground")}>{topic.title}</p>
                                            {isCompleted && <CheckCircle2 className="shrink-0 h-3.5 w-3.5 text-emerald-500" />}
                                          </Link>
                                        );
                                      })}
                                    </div>
